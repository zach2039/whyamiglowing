...
public class AddRadiationContaminationCommand {
	static ArgumentBuilder<CommandSourceStack, ?> register() {
		return Commands.literal("add_contamination").requires((sourceStack) -> sourceStack.hasPermission(4))
	...
	private static int execute(final CommandContext<CommandSourceStack> context, final Entity entity, final float amount) throws CommandSyntaxException {
		...
		radiation.increaseContaminationMilliremsPerSecond(RadiationHelper.convertPerHourToPerSecond(amount));

		context.getSource().sendSuccess(
				Component.translatable(
						WhyAmIGlowingLang.MESSAGE_CONTAMINATION_ADD.getTranslationKey(),
						entity.getDisplayName(),
						RadiationHelper.getDosageDisplayMilliremsPerHour(amount)
				),
				true
		);
