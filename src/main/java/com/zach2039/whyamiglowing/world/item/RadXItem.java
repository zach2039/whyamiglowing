...
public class RadXItem extends Item {
	...
	public RadXItem(Properties arg) {
		super(arg);
	}
	...
	public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> tooltip, TooltipFlag tooltipFlag) {
		...
		TooltipHelper.appendShiftTooltip(tooltip, WhyAmIGlowingLang.ITEM_RAD_X_DESC.getTranslationKey());

		String radiationResistanceDuration = String.format("(%.1f sec)", RadiationHelper.getRadXDurationTicks() / 20f);
		String radiationResistancePercentage = String.format("-%.1f%%", RadiationHelper.getRadXExposureResistance() * 100f);
		tooltip.add(Component.literal(""));
		tooltip.add(Component.literal( "☢: " + radiationResistancePercentage + " " + radiationResistanceDuration).withStyle(ChatFormatting.DARK_GREEN));
	}

	private void applyEffectToTarget(final Player player, final LivingEntity target) {
		int amp = 0;
		MobEffectInstance prevEffect = player.getEffect(ModMobEffects.RAD_X_EFFECT.get());

		// Allow stacking of effect
		if (prevEffect != null)
			amp = Math.min(RadiationHelper.getRadXMaxLevel(), prevEffect.getAmplifier() + 1);

		// Apply rad-x
		target.addEffect(new MobEffectInstance(ModMobEffects.RAD_X_EFFECT.get(), RadiationHelper.getRadXDurationTicks(), amp));
